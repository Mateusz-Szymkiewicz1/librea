...
function Collection(props) {
  ...
  const deleteBook = async (ev) => {
    ...
    if(collection.books.length < 2){
      props.setToast({type: "error", text: "You can't have an empty collection!"})
      return;
    }
    ...
    fetch("http://localhost:3000/collection_delete_book", {
      ...
      props.setToast({type: "msg", text: "Deleted a book!"})
  ...
  const editInfo = async () => {
    ...
    if (document.querySelector(".decision")) document.querySelector('.decision').remove()
      ...
      fetch("http://localhost:3000/collection_edit_info", {
        ...
        setCollection(structuredClone(collection))
        props.setToast({type: "msg", text: "Edited collection's info!"})
        closeEdit()
      })
  }
  const closeEdit = () => {
  ...
  const deleteCollection = async () => {
    if (document.querySelector(".decision")) document.querySelector('.decision').remove()
      ...
      fetch("http://localhost:3000/collection_delete", {
        ...
        props.setToast({type: "msg", text: "Deleted a collection!", stay: true})
        navigator('/')
      })
  }
  return (
                ...
                return (
                  <span key={el.id} onClick={(e) => {
                    ...
                    if(!collection.books.find(x => x.id == el.id)){
                      ...
                      setCollection(structuredClone(collection))
                      props.setToast({type:"msg", text: "Added a book to the list!"})
                      addBook({id: el.id})
                    }else{
